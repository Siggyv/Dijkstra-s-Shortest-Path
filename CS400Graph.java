...
public class CS400Graph<NodeType, EdgeType extends Number> implements GraphADT<NodeType, EdgeType> {
  ...
  protected class Path implements Comparable<Path> {
    ...
    public Path(Path copyPath, Edge extendBy) {
      ...
      // start remains the same between each path
      this.start = copyPath.start;
      // adds the original path distance and then considers with the added edge.
      this.distance = copyPath.distance + extendBy.weight.doubleValue();
      // making deep copy of the data sequence
      // initialize first
      this.dataSequence = new LinkedList<>();
      for (NodeType node : copyPath.dataSequence) {
        this.dataSequence.add(node);
      }
      // adds the target data value to the sequence.
      this.dataSequence.add(extendBy.target.data);
      // makes the end the extended target of the edge.
  ...
  protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
    if (vertices.get(start) == null || vertices.get(end) == null) {
      throw new NoSuchElementException("Could not find start or end vertices.");
    }
    //creating priorityqueue for each added path
    PriorityQueue<Path> pq = new PriorityQueue<>();
    //starting with start -> start
    pq.add(new Path(vertices.get(start)));
    //hashtable for shortest paths discovered, key= data/NodeType and value is the shortest path
    Hashtable<NodeType, Path> pathsFound = new Hashtable<>();
    //while loop for whenever the queue is not empty (i.e. all possible shortest paths found)
    while(!pq.isEmpty()) {
      Path path = pq.peek();
      //if the end of the current path doesn't have any leaving edges don't add to queue.
      if(path.end.edgesLeaving != null) {
        //if there is no entry for the current end node, then it will add it 
        if(!pathsFound.containsKey(path.end.data)) {
          pathsFound.put(path.end.data, path);
        }else {
          //remove the non-shortest path and continue to next node
          pq.remove(path);
          continue;
        }
        //add all new things to find the path found.
        for(Edge v: path.end.edgesLeaving) {
          pq.add(new Path(path, v));
        }
        //remove the shortest path
        pq.remove(path);
      }
    }
    if(pathsFound.get(end) == null) {
      throw new NoSuchElementException("No path found...");
    }
    return pathsFound.get(end);
  }
